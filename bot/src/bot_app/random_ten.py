...
async def train_ten(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await message.reply(
            f"{data['step']} of 10. Das word ist {data['word']}", reply_markup=inline_kb
        )


@dp.callback_query_handler(
    lambda c: c.data in ["das", "die", "der"], state=GameStates.random_ten
)
        ...
        if answer == data["answer"]:
            ...
            if data["step"] > 10:
                await bot.send_message(
                    callback_query.from_user.id, "The game is over!!!"
                )
                await GameStates.start.set()
            else:
                await bot.send_message(
                    callback_query.from_user.id,
                    "Ya\n" + f"{data['step']} of 10. Das word ist {data['word']}",
                    reply_markup=inline_kb,
                )
        else:
            await bot.send_message(
                callback_query.from_user.id, f"Nein\n", reply_markup=inline_kb
            )
