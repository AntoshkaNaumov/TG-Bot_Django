...
async def train_all(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await message.reply(
            f"{data['step']}. Das word ist {data['word']}", reply_markup=inline_kb
        )


@dp.callback_query_handler(
    lambda c: c.data in ["das", "die", "der"], state=GameStates.all_words
)
async def button_click_call_back_all(
        ...
        if answer == data["answer"]:
            ...
            if res:
                ...
                await bot.send_message(
                    ...
                    f"{data['step']}. Das word ist {data['word']}",
